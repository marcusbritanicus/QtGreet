...
// creating an empty element in the Map with that key.
        class MapProxy : public Value {
            ...
            private:
                ...
                MapProxy( std::shared_ptr<ValueImpl> parent, const std::string& key, Value *pTarget );
...
// "options". Throws Hjson::file_error if the file cannot be opened for writing.
        void MarshalToFile( const Value& v, const std::string& path, const EncoderOptions& options = EncoderOptions() );
...
// Creates a Value tree from input text.
        Value Unmarshal( const char *data, size_t dataSize, const DecoderOptions& options = DecoderOptions() );
...
// Creates a Value tree from input text.
        Value Unmarshal( const std::string& data, const DecoderOptions& options = DecoderOptions() );

// Reads the entire file (in binary mode) and unmarshals it. Throws
// Hjson::file_error if the file cannot be opened for reading.
        Value UnmarshalFromFile( const std::string& path, const DecoderOptions& options = DecoderOptions() );
